...
El cluster fue creado atraves de AWS CLI, en estecaso atravez de AWS Cloud Shell  para esto primero (asumiedo que ya se tienen una key pair a utilizar) se crea un bucket en S3 usando el comando
```bash
...
Apartir de aquihay que entrar a el cluster ERM atravez del siguiente comando
```bash
cd <Donde tengas tu key>
ssh -i ./emr-key.pem.pem hadoop@ec2-18-215-182-149.compute-1.amazonaws.com
```
Aparte de esto se instalar lo siguiente: Git, mrjob y pip atravez de los siguientes comandos
```bash
sudo yum update
